...
Target detection uses measured line width instead of accepting a single sensor sample. A target candidate is a centered wide reading with black on both left and right sides, including patterns such as `01110`, `01111`, `11110`, or `11111`. When such a reading appears, the robot slows down to `LINE_WIDTH_SCAN_SPEED`, keeps driving forward, and counts how many 20 ms ticks a wide line or intersection reading remains present. If the measured width reaches `TARGET_WIDTH_MIN_TICKS`, the marker is confirmed as the target. If the measured width is smaller, the reading is treated as a normal intersection or line feature; for a left intersection, the robot then performs the 90-degree left turn and records `L` only after the new line is successfully reacquired. Left intersections that are not target candidates also measure and print their line width before turning. If a left-intersection width measurement reaches `TARGET_WIDTH_MIN_TICKS`, the robot confirms the target immediately instead of recording `L` and continuing. This keeps target detection tied to the physical width of the black marker and keeps path memory tied to movements that were actually completed.
