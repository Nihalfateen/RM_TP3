...
Return navigation currently uses a temporary final condition: after all return-path moves have been executed, the robot follows the line for a tunable final straight segment (`RETURN_FINISH_FORWARD_TICKS`) and then stops. This is an improvement over stopping immediately after the last return decision, but it still does not prove the robot has reached the exact physical start point. Accurate final stopping requires either a detectable start marker or calibrated localization/odometry.
