...
Target detection uses a stable wide-line pattern instead of a single sensor sample. A candidate target is detected when the reading has center coverage plus black on both the left and right sides, so patterns such as `01110` can be accepted when the physical target marker does not cover all five sensors. To avoid false positives while the robot is spinning, recovering, or stuck at the wrong end, target confirmation is armed only after `TARGET_READY_TICKS` stable line-following readings. During confirmation, the robot slows down to `TARGET_SCAN_SPEED` and checks that the same core target pattern remains stable for `TARGET_CONFIRM_TICKS`; the target is confirmed only if this stable pattern appears at least `TARGET_CONFIRM_MIN_HITS` times. If the scan does not collect enough stable hits, the candidate is rejected and normal exploration continues.
