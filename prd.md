...
Target detection uses a stable wide-line pattern instead of a single sensor sample. A candidate target is detected only when at least four of the five ground sensors see black while the reading includes center, left-side, and right-side coverage. To avoid false positives while the robot is spinning, recovering, or stuck, target confirmation is armed only after `TARGET_READY_TICKS` stable line-following readings. Because the robot may pass over a thick target marker too quickly at normal line-following speed, the current implementation slows down to `TARGET_SCAN_SPEED` and performs a short confirmation scan for `TARGET_CONFIRM_TICKS`. The target is confirmed only if the wide-line candidate appears at least `TARGET_CONFIRM_MIN_HITS` times during that scan. If the scan does not collect enough hits, the candidate is rejected and normal exploration continues.
