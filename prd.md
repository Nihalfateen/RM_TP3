...
- Milestone 4 is completed: exploration movements are now recorded in a compact fixed-size path buffer after physical execution. The current implementation measures line width at a left intersection, enters the junction, executes the 90-degree left turn, reacquires the new line with the center sensor, and only then records `L`. Line sensors remain the main navigation reference, while encoder odometry is used as a guardrail: turns must rotate through an expected angle range, and intersection detection stays disarmed until the robot has moved a minimum odometry distance away from the junction and observed a stable normal single-line pattern for several ticks. If the turn fails or the new line is not acquired, the move is not recorded. The buffer prevents overflow and the recorded path is printed when the target is confirmed.
...
Target detection uses measured line width as part of intersection handling, instead of scanning or counting target-like readings during normal line following. The robot follows the line normally until a left intersection is detected. At that point it measures how many 20 ms ticks a wide line or intersection reading remains visible while moving slowly. If that measured width reaches `TARGET_WIDTH_MIN_TICKS`, the robot confirms the final wide black marker as the target and stops exploration. If the measured width is smaller, the robot treats the reading as a normal intersection, performs the 90-degree left turn, and records `L` only after the new line is successfully reacquired. After the turn, the robot waits for an intersection-cleared state before re-arming detection; this state now requires both stable normal-line sensor readings and a minimum encoder-estimated distance from the handled junction. This keeps target detection tied to the physical width of the black marker, avoids repeated stop/start behavior on ordinary wide readings, and keeps path memory tied to movements that were actually completed.
