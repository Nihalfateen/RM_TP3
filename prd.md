...
- Milestone 4 is completed: exploration movements are now recorded in a compact fixed-size path buffer after physical execution. The current implementation debounces possible left intersections for several consecutive ticks before approaching the junction, validates that the robot is centered on a stable intersection pattern, measures line width, executes the 90-degree left turn, reacquires the new line with the center sensor, and only then records `L`. Unstable patterns such as non-centered readings are rejected, and zero or too-small width measurements are not allowed to trigger a turn. After handling a junction, intersection detection stays disarmed until the robot has fully cleared the junction and observed a stable normal single-line pattern for several ticks. If clearing times out, the detector is safely re-armed with a warning. If the turn fails or the new line is not acquired, the move is not recorded. The buffer prevents overflow and the recorded path is printed when the target is confirmed.
...
- Milestone 6 is completed: after target confirmation, the robot builds a fixed-size return path from the optimized exploration path, stops at the target, and waits for a second start-button press before beginning the return phase. Once return mode starts, it turns around, follows the line, debounces return-intersection candidates, and executes one return move at each validated detected intersection. The return path is constructed by reading the optimized path backwards and inverting each move: `L` becomes `R`, `R` becomes `L`, while `S` and `B` remain unchanged. The implementation prints the return path and each executed return move, keeps stop-button checks active during forward approach and turn loops, and prevents return-path buffer overflow.

Target detection uses measured line width as part of validated intersection handling, instead of scanning or counting target-like readings during normal line following. The robot follows the line normally until a possible left intersection is detected for `INTERSECTION_DETECT_TICKS` consecutive ticks, then approaches the junction and validates the reading before acting. At that point it measures how many 20 ms ticks a wide line or intersection reading remains visible while moving slowly. If that measured width reaches `TARGET_WIDTH_MIN_TICKS`, the robot confirms the final wide black marker as the target and stops exploration. If the measured width is smaller but still at least `INTERSECTION_WIDTH_MIN_TICKS`, the robot treats the reading as a normal intersection, performs the 90-degree left turn, and records `L` only after the new line is successfully reacquired. After the turn, the robot waits for an intersection-cleared state before re-arming detection, so repeated sensor patterns inside the same physical junction are not recorded as duplicate moves. This keeps target detection tied to the physical width of the black marker, avoids repeated stop/start behavior on ordinary wide readings, and keeps path memory tied to movements that were actually completed.
