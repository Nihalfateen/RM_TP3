...
- `probe-target-v9-encoder-return`
...
The implementation probes target-like wide readings immediately while line-following and while clearing intersections after turns. On the first wide reading, the robot slows down, measures the physical width in 20 ms ticks, and confirms the target if the measured width reaches `TARGET_WIDTH_MIN_TICKS`. After a confirmed wide marker, the robot backs up slightly so it stops closer to the target instead of after it.
...
- `TARGET_WIDTH_MIN_TICKS`: minimum measured wide-line duration needed to confirm the target. Current value: `8`.
- `TARGET_BACKUP_MAX_TICKS`: maximum reverse correction after target confirmation.
...
- The target is confirmed by immediately probing target-like readings and accepting them when the measured width reaches `TARGET_WIDTH_MIN_TICKS`.
